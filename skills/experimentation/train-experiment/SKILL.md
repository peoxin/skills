...
Read the fixed Experiment definition, the Dataset inputs declared for the Train phase, Model component revision, optional Benchmark revision, phase configuration, dependency identity, resource assignment, and execution context. Verify that the declared training inputs are available and that the output checkpoint path or locator is writable.

Before direct execution, require the Experiment control commit and the Git preflight described by `$run-experiment`: consistent `EXPERIMENT.md` and declared configuration or phase implementation, clean source paths, complete component SHAs from the Experiment control commit message, existing commits, and current checkout paths matching each component commit. When delegated, consume the orchestrator's verified context rather than repeating or weakening it.
