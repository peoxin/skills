...
results/       # execution outputs and user-selected artifacts
```
...
An Experiment spec may contain independent `train` and `evaluate` phases. `run-experiment` executes only the phases declared in the spec, in their declared order, using the environment configuration, commands, and outputs already fixed by `define-experiment`. It does not accept temporary phase, environment, command, or output overrides. A train-only Experiment requires a Model and Dataset derivations and may omit a Benchmark; an evaluation-only Experiment requires a Model, Benchmark, and checkpoint. Training produces the outputs declared for Train; evaluation consumes the checkpoint and Benchmark inputs declared for Evaluate. Missing phases or declared outputs are never silently treated as success.
