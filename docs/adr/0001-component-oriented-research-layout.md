...
The target research project uses visible root-level component directories: `sources/`, `data/`, `models/`, `benchmarks/`, `experiments/`, and `results/`. A Benchmark is a self-contained data-and-metric protocol that may compose multiple Dataset setups and is owned by `define-benchmark`; an Experiment consumes a confirmed Benchmark, contains its optional training settings and phases, and is owned by `define-experiment`; a Result records one execution. We deliberately do not create top-level `training/`, `evaluation/`, `metrics/`, `runs/`, `reports/`, or shared model-code directories because these concepts belong to their owning component or phase.
