...
Logical directory names are stable identities rather than versions. A formal Experiment control commit fixes the Experiment and its declared same-repository component dependency closure. A same-repository component is identified by that commit SHA and path. Large data or execution artifacts may remain outside Git when their provenance and digest are recorded.
